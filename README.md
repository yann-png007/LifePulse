...
*   **Redis (NoSQL 数据库)**: 作为高性能的二级缓存、分布式锁和原子计数器，应对高并发读写场景。
*   **Caffeine (本地缓存)**: 作为一级缓存，提供毫秒级的内存访问速度，进一步提升热点数据读取性能。

1.  用户请求通过 Nginx 负载均衡进入 Spring Boot 应用。
2.  **读请求**：优先访问 **Caffeine** 本地缓存，若未命中，则访问 **Redis** 二级缓存。若仍未命中，则查询 **MySQL**，并将结果回写至 Redis 和 Caffeine。
...
| **缓存**     | Redis, Caffeine        | 多级缓存体系，提供极致读取性能          |
| **分布式组件**  | Redisson               | 基于 Redis 的高级分布式锁与布隆过滤器实现 |
| **消息队列**   | RabbitMQ               | 异步任务处理、流量削峰              |
| **认证授权**   | JWT (JSON Web Token)   | 无状态、可扩展的用户认证与授权方案        |
| **安全**     | BCrypt                 | 专业的密码哈希加密算法              |
| **其他**     | Lombok, Slf4j, Knife4j | 简化代码、日志记录、API文档          |
...
*   **统一用户模块**：
    *   支持**手机号+密码**和**手机号+短信验证码**双重登录。
    *   未注册手机号**自动注册**新用户，并生成默认凭证。
    *   基于 `ThreadLocal` 的 `UserContext` 实现优雅的用户信息传递。
*   **商户模块**：商户信息的增删改查、分页查询。
...
针对现代应用的用户习惯，重构了用户认证体系：
1.  **手机号作为唯一标识**：简化了用户身份模型，便于整合短信服务。
2.  **双重登录方式**：提供了密码和短信验证码两种登录途径，提升了用户体验。
3.  **无感知自动注册**：新用户首次使用手机号登录时，系统在后台通过**分布式锁**保证线程安全，自动为其创建账户。此设计降低了用户注册门槛，有效提升了用户转化率。

### 5.2. 健壮的多级缓存与防护体系

基于 **Caffeine**、**Redis** 和 **Redisson** 构建了完善的缓存系统，并解决了缓存应用中的经典难题：

*   **多级缓存 (`Caffeine` + `Redis`)**：构建了“本地缓存+远程缓存”的两级体系，极大提升了热点数据的访问速度，降低了对 Redis 的访问压力。
*   **缓存穿透**：引入 **Redisson 分布式布隆过滤器**。在应用启动时预热所有用户手机号，能够以极高的效率过滤掉绝大部分不存在的用户请求，从根源上防止恶意穿透攻击。
...
*   **代码质量**：遵循 **YAGNI (You Ain't Gonna Need It)** 原则，持续重构并移除了项目中的“死代码”和“魔法数字”，并通过 `UserContext` 等设计提升了代码的可读性和可维护性。
